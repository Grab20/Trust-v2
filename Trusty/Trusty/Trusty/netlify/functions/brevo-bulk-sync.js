...
async function buildDataset(sb) {
  ...
  // Fetch approved driver profile IDs first, then filter
  const { data: approvedDpRows } = await sb
    .from('driver_profiles')
    .select('user_id')
    .eq('status', 'approved');
  const approvedDriverIdSet = new Set((approvedDpRows || []).map(d => d.user_id));

  // Only sync approved users: approved drivers OR approved owners
  const syncProfiles = allProfiles.filter(p => {
    const role = p.role || 'driver';
    const isDriver = role === 'driver' || role === 'both';
    const isOwner  = role === 'owner'  || role === 'both';
    if (isDriver && approvedDriverIdSet.has(p.id)) return true;
    if (isOwner  && p.admin_status === 'approved') return true;
    return false;
  });

  const ids = syncProfiles.map(p => p.id);
  if (!ids.length) return { allProfiles: syncProfiles, dpMap: {}, carsMap: {}, matchedDriverIds: new Set(), matchedOwnerIds: new Set(), activeBoostMap: {} };
  ...
  return { allProfiles: syncProfiles, dpMap, carsMap, matchedDriverIds, matchedOwnerIds, activeBoostMap };
}
...
exports.handler = async function (event) {
  ...
  if (!SUPABASE_URL || !SUPABASE_SERVICE_KEY) {
    return { statusCode: 500, headers: CORS, body: JSON.stringify({ error: 'Missing SUPABASE_URL or SUPABASE_SERVICE_ROLE_KEY env vars' }) };
  }
  ...
  return {
    ...
    console.error('brevo-bulk-sync error:', err);
