...
const FROM_EMAIL           = 'TrustMate <noreply@trustmate.co.za>';
...
exports.handler = async function (event) {
  ...
  // GET ping — confirms function is reachable
  if (event.httpMethod === 'GET') {
    return { statusCode: 200, headers: { ...CORS, 'Content-Type': 'application/json' }, body: JSON.stringify({ ok: true, fn: 'send-campaign' }) };
  }
  ...
  console.log('camp auth: secret_set='+!!ADMIN_SECRET+' match='+(secret===ADMIN_SECRET));
  if (!ADMIN_SECRET || secret !== ADMIN_SECRET) {
    return { statusCode: 403, headers: { ...CORS, 'Content-Type': 'application/json' }, body: JSON.stringify({ error: 'Forbidden — wrong or missing secret key', hint: !ADMIN_SECRET ? 'ADMIN_CAMPAIGN_SECRET env var not set' : 'Secret key does not match' }) };
  }
  ...
  let sent = 0, failed = 0, failedList = [], resendError = null;
  ...
  for (let i = 0; i < recipients.length; i += BATCH_SIZE) {
    const batch = recipients.slice(i, i + BATCH_SIZE).map(r => {
      ...
      return {
        ...
        to: [r.email],
        subject,
        text: textBody.replace(/\{\{name\}\}/g, firstName),
      };
    });
    ...
    try {
      ...
      if (res.ok) {
        ...
        resendError = data;
        failed += batch.length;
        failedList.push(...batch.map(e => e.to[0]));
      }
    } catch (e) {
  ...
  return {
    statusCode: 200,
    headers: { ...CORS, 'Content-Type': 'application/json' },
    body: JSON.stringify({ ok: true, sent, failed, failedList, resendError }),
  };
};
